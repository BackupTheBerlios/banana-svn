...
void gg_system_draw_filled_rect(int x, int y, int width, int height, gg_colour_t *colour)
{
    ...
    if (clip)
    {
        ...
        driver->draw_filled_rect(dest_c.x, dest_c.y, dest_c.width, dest_c.height, colour, colour, colour, colour, FALSE);
    }
    else
        driver->draw_filled_rect(x, y, width, height, colour, colour, colour, colour, FALSE);
}
    ...
    gg_colour_t *top_left, gg_colour_t *top_right, gg_colour_t *bottom_left, gg_colour_t *bottom_right, int start_right )
{
    ...
    if (clip)
    {
        ...
        dest.height = height;
        dest_c = gg_clipping_rect(&dest, clip);
        driver->draw_filled_rect(dest_c.x, dest_c.y, dest_c.width, dest_c.height, top_left, top_right, bottom_left, bottom_right, start_right );
    }
    else
        driver->draw_filled_rect(x, y, width, height, top_left, top_right, bottom_left, bottom_right, start_right );
}
...
void gg_system_draw_image(void *image, gg_rect_t source, gg_rect_t dest, int mode_h, int mode_v, gg_colour_t *colour, int glow)
{
    ...
    if (clip)
    {
        ...
        driver->draw_image(image, source_c, dest_c, mode_h, mode_v, colour, glow);
    }
    else
        driver->draw_image(image, source, dest, mode_h, mode_v, colour, glow);
}
